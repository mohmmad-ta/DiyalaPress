...
const putUpdateNews = async (id)=>{
  const dataPost = new FormData();
  dataPost.append('title', store.newsData.title);
  dataPost.append('content', store.newsData.content);
  dataPost.append('sendNoifyForGuest', store.newsData.sendNoifyForGuest);
  dataPost.append('image', store.newsData.image);
  const res = await axios.put(`api/news/${id}`, dataPost, {
    headers: {
      'Content-Type': 'multipart/form-data',
    },
  })
...
const News = async () =>{
  ...
  if (store.form_status === "post"){
    ...
    await putUpdateNews(store.alert.id)
    store.openForm = 0
  }
}
  ...
  <div class="w-full overflow-hidden justify-center">
    <form class="text-right">
      ...
      <div class="flex justify-center pt-8 pb-4 items-center gap-4">
        <button @click="store.openForm = 0" type="button" class="w-3/4 bg-secondary-500 py-1.5 font-bold text-white rounded">الغاء</button>
        <button @click="News()" type="button" class="w-3/4 bg-main-50 py-1.5 font-bold text-white rounded">تاكيد</button>
      </div>
    </form>
  </div>
</template>