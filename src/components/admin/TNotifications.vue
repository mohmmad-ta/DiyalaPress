...
<template>
  <div class=" h-full w-full bg-white p-2.5 rounded">
    <table class="table-fixed w-full">
      ...
      <tr :class="index % 2 !== 0 ? 'bg-secondary-100' : 'bg-none'" v-for="(notifications, index) in store.notifications" :key="index">
        <td class="py-3 tableHid">{{ notifications.created_at }}</td>
        <td>{{ notifications.data.message.length > 20 ? notifications.data.message.slice(0,20) + " ... " : notifications.message.data }}</td>
